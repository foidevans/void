  ...
  useEffect(() => {
    ...
    async function connectSocket() {
      ...
      const manager = new SocketManager(token, {
        ...
        onError: (detail) => {
  if (detail !== "WebSocket connection error.") {
      ...
      <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          {onBack && (
            <button
              onClick={onBack}
              className="flex h-9 w-9 items-center justify-center rounded-[8px] text-[var(--text-secondary)] transition-all hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)] md:hidden"
            >
