...
export function UserSearch({ onSelectUser }: UserSearchProps) {
  ...
  return (
    <div className="border-b border-[var(--border)] bg-[var(--bg-sidebar)] px-4 pb-4 pt-4">
      {/* Label */}
      <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--accent-text)]">
        Start a new conversation
      </p>

      {/* Search input */}
      <div className="relative">
        <div className="flex items-center gap-2 rounded-[10px] border border-[var(--border-focus)] bg-[var(--accent-dim)] px-3 py-2.5">
          <span
            className="material-symbols-outlined text-[var(--accent-text)]"
            style={{ fontSize: 18 }}
          >
            {isSearching ? "sync" : "person_search"}
          </span>
        ...
        {/* Results dropdown */}
        {results.length > 0 && (
          <ul className="absolute left-0 right-0 top-full z-20 mt-1 overflow-hidden rounded-[10px] border border-[var(--border)] bg-[var(--bg-surface)] shadow-lg">
            {results.map((user) => (
              <li key={user.id}>
                <button
                  onClick={() => handleSelect(user)}
                  className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[var(--bg-hover)]"
                >
                  <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-[#6c3bff] to-[#a855f7] text-[12px] font-semibold text-white">
                    {user.display_name.slice(0, 2).toUpperCase()}
                  </div>
                  <div>
                  ...
                  <span
                    className="material-symbols-outlined ml-auto text-[var(--accent-text)]"
                    style={{ fontSize: 18 }}
                  >
                    chat_bubble
                  </span>
